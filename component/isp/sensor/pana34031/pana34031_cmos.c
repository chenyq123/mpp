...
static cmos_isp_default_t st_coms_isp_default =
{
    ...
    {
        5000,
    	{	0x018f, 0x8061, 0x802e,
    		0x803e, 0x01e7, 0x80a9,
    		0x0027, 0x8095, 0x016e
    	},
        3200,
    	{	0x018f, 0x8061, 0x802e,
    		0x803e, 0x01e7, 0x80a9,
...
{
    cmos_inttime.full_lines_std = 718;
	cmos_inttime.full_lines_std_30fps = 719;
	cmos_inttime.full_lines = 718;
	cmos_inttime.full_lines_del = 718; //TODO: remove
	cmos_inttime.full_lines_limit = 65535;
	cmos_inttime.max_lines_target = 718;
	cmos_inttime.min_lines_target = 2;
	cmos_inttime.vblanking_lines = 718;
		...
		HI_U32 exposure_shift)
{
	...
	HI_U32 _again_db = 0;
	HI_U32 exposure1,exposure2,exposure3;
	int shft = 0;
...
/*
     exposure1 = (exposure >> 1);
        ...
        exposure3 = (exposure >> 3);
        printf("exposure=%d  exp_max=%d  exposure3=%d  ",exposure, exposure_max,exposure3);
         if((exposure3 > exposure_max) )
         {
            _again *= 8;
            _again_db = 18;
            exposure = exposure3;
         ...
         else if((exposure2 > exposure_max) )
         {
            _again *= 4;
            _again_db = 12;
            exposure = exposure2;
         }
         else if((exposure1 > exposure_max) )
         {
            _again *= 2;
            _again_db = 6;
            exposure = exposure1;
...
//digital  step = 0.134db,every step means 0.134db, the maximum  value of _res is  179
//是否需要加入四舍五入运算
		...
		HI_U32 exposure_shift)
{
    ...
    //HI_U32 _dgain_db = 0;
 //   printf("dgain_shift=%d , _dgain=%d  ",p_gains->dgain_shift,_dgain);
    int shft = 0;
    ...
    printf("dgain_db=%d ",p_gains->dgain_db);
