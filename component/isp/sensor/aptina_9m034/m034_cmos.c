...
{
    //sharpen_alt_d
    {0x50,0x4a,0x45,0x40,0x3a,0x38,0x34,0x30},

    //sharpen_alt_ud
    {0x90,0x88,0x80,0x78,0x70,0x64,0x50,0x30},

    //snr_thresh
    {0x05,0x08,0x0c,0x10,0x18,0x20,0x28,0x30},
...
static /*__inline*/ cmos_inttime_const_ptr_t cmos_inttime_initialize()
{
    ...
    switch(gu8SensorMode)
    {
        ...
        case 1: //WDR mode for T1 exposure, Exposure ratio = 16X16X
            cmos_inttime.max_lines = 672;
            cmos_inttime.min_lines = 128;
            cmos_inttime.max_lines_target = 672;
		...
		)
{
    ...
    //if(1 == gu8SensorMode) /*WDR mode, isp dgain*/
    {
        ...
        exposure = (exposure0 << p_gains->dgain_shift) / p_gains->dgain;
        if(exposure > exposure_max)
    	{
            exposure_max = DIV_0_TO_1(exposure_max);
...
static __inline HI_U32 cmos_get_ISO(cmos_gains_ptr_t p_gains)
{
    ...
    if (gu8SensorMode) /* WDR mode */
    {
        if (p_gains->iso > 800)
        {
            p_gains->iso = p_gains->iso >> 3;
        }
        else
        {
            p_gains->iso = 100;
