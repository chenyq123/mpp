...
HI_S32 SAMPLE_COMM_VENC_SaveSnap(VENC_STREAM_S *pstStream)
{
    ...
    sprintf(acFile, "/home/assets/images/snap_%s.jpg",timestr);

#if 1
    if(-1 == system("rm -r /home/assets/images/*"))
    {
        printf("rm error!\n");
    }
#endif
...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID *p)
{
    ...
    //VIDEO_FRAME_INFO_S pstVideoFrame1;
    ...
    ******************************************/
#if 0
    s32Ret=HI_MPI_VPSS_UserGetFrame(0, 1,&pstVideoFrame1);
    if(s32Ret!=HI_SUCCESS)
    {
    	printf("HI_MPI_VPSS_UserGetFrame fail with err code %#x!\n",s32Ret);
    }
#endif
...
//	printf("%s\n",asctime(gmtime(&timep)));
        sprintf(aszFileName[i], "/home/video/%sstream_chn%d%s",timestr,i, szFilePostfix);
