...
#define MULTI_BASE 0x200F0000

#define I2C_SDA_MUXCTRL IO_ADDRESS(MULTI_BASE + 0x18)
#define I2C_SCL_MUXCTRL IO_ADDRESS(MULTI_BASE + 0X1C)
...
long gpioi2c_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
        ...
        case GPIO_I2C_SDA_UP:
            i2c_set(SDA);
            break;

        case GPIO_I2C_SDA_DOWN:
            i2c_clr(SDA);
            break;
...
ssize_t read(struct file *filp, char __user *buff, size_t count, loff_t *offp)
{
    ...
    copy_to_user(buff,buf,count);
    kfree(buf);
    return num;
}

ssize_t write(struct file *file, const char __user *buff, size_t count, loff_t *offp)
{
    char *buf = kmalloc(count,GFP_KERNEL);
...
static int __init gpio_i2c_init(void)
{
    ...
    i2c_set(SCL | SDA);
    printk("%d\n",*(int *)I2C_SDA_MUXCTRL);
    *(int*)(I2C_SDA_MUXCTRL) = 0;
    *(int*)(I2C_SCL_MUXCTRL) = 0;
    printk("%d\n",*(int *)I2C_SDA_MUXCTRL);
    printk("%d\n",*(int *)I2C_SCL_MUXCTRL);
